...
def reconstruct_model_predict(model, device, image_data, pseudo_label_data):
    """Passes padded 3D subject through VAE, discarding latent variables to yield raw reconstruction."""
    ...
    # The VAE expects the MRI image and the mask to be stacked as channels
...
def validate_checkpoint(checkpoint_path, base_log_dir, fold_source_dir):
    """Executes full 5-fold validation on a checkpoint to verify generalization."""
    cp_name = os.path.basename(checkpoint_path).replace('.pth', '')
    run_log_dir = os.path.join(base_log_dir, "vae", cp_name)
    ...
    print(f"\n{'='*50}")
    print(f"STARTING VALIDATION: {cp_name} (vae)")
    print(f"{'='*50}")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = VAE3D(in_channels=2, out_channels=1).to(device)

    # 1. Boot up the architecture with the requested weights
    try:
    ...
    for i in range(5):
        ...
        for dp_id in all_ids:
            ...
            try:
                ...
                # so the heavy black background doesn't massively skew the MRI contrast
                ...
                # --- 3D Network Padding ---
                # The UNet/VAE requires shapes cleanly divisible by 16/32
                ...
                # Center crop if the image is somehow larger than the target
                ...
                # Reverse the padding to get the prediction back into the original subject shape
...
def main():
    ...
    # Target checkpoints we want to scientifically validate
    vae_cps = [
        r"C:\Users\Lab\OneDrive\Desktop\SPQA\logs\best_cps\Run_125\vae_epoch_2440.pth",
        r"C:\Users\Lab\OneDrive\Desktop\SPQA\logs\best_cps\Run_125\vae_epoch_2480.pth",
        r"C:\Users\Lab\OneDrive\Desktop\SPQA\logs\best_cps\Run_125\vae_epoch_2420.pth"
    ]
    ...
    print("--- STARTING VAE BATCH VALIDATION ---\n")
    for cp in vae_cps:
