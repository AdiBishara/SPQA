...
def morphological_corrupt(mask):
    ...
    - Severity: 1-10 random iterations (widened for better QC calibration)

    Wider range forces the model to see severely corrupted inputs,
    teaching it to produce proportionally poor reconstructions for poor
    pseudo-labels, improving the Pearson/Spearman QC correlation metric.
    """
    corr = mask.clone()
    iterations = torch.randint(1, 11, (1,)).item()
...
class PhaseManager:
    ...
    def get_weights(self):
        """Returns the current active (or blended) weights."""
        name, thresh, target = self.phases[self.locked_idx]

        if not self.blending:
            return target

        # Linear blending between previous and target weights
        alpha = self.blend_epoch / BLEND_EPOCHS
        blended = {}
        for k in target.keys():
            blended[k] = (1 - alpha) * self.prev_weights[k] + alpha * target[k]

        return blended

    def step(self, ema_dice):
        """Updates internal state based on latest smoothed performance."""
        # Check if we should advance to the next phase
        next_idx = self.locked_idx + 1
        if next_idx < len(self.phases):
            next_name, next_thresh, next_weights = self.phases[next_idx]

            if ema_dice >= next_thresh:
                self.confirm_counter += 1
                if self.confirm_counter >= CONFIRM_EPOCHS:
                    # LATCH: Phase confirmed!
                    self.prev_weights = self.get_weights()  # Start from current
                    self.target_weights = next_weights
                    self.locked_idx = next_idx
                    self.confirm_counter = 0
                    self.blending = True
                    self.blend_epoch = 0
                    return self.phases[self.locked_idx][0], f"*** PHASE ADVANCED: {next_name} (Threshold: {next_thresh}) ***"
            else:
                self.confirm_counter = 0

        # Progress blending if active
        if self.blending:
            self.blend_epoch += 1
            if self.blend_epoch >= BLEND_EPOCHS:
                self.blending = False
                return self.phases[self.locked_idx][0], f"*** BLEND COMPLETE: fully in {self.phases[self.locked_idx][0]} ***"
            else:
                current_target_name = self.phases[self.locked_idx][0]
                prev_name = self.phases[self.locked_idx - 1][0]
                return current_target_name, f">>> BLENDING: {prev_name} -> {current_target_name} [epoch {self.blend_epoch}/{BLEND_EPOCHS}, \u03b1={self.blend_epoch/BLEND_EPOCHS:0.2f}]"

        return self.phases[self.locked_idx][0], None
...
def train_dae():
    ...
    log_dir, save_dir, vis_dir = [os.path.join(r"C:\Users\Lab\OneDrive\Desktop\SPQA\logs", d) for d in ["training_logs", "dae_checkpoints", "visual_progress"]]
