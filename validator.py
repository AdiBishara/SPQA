...
def main():
    ...
    # Checkpoint: CLI arg or default
    if args.checkpoint:
        model_path = args.checkpoint
    else:
        model_path = os.path.join(base_path, "logs", "best_cps", "run_22_best_cps", "vae_epoch_733.pth")
    ...
    for i in range(5):
        ...
        all_ids = [d for d in os.listdir(pseudo_labels_dir)
                   if os.path.isdir(os.path.join(pseudo_labels_dir, d))]
        ...
        for dp_id in all_ids:
            ...
            try:
                ...
                all_subject_data.append({
                    'Fold': fold_name, 'ID': dp_id,
                    'Actual': act_d, 'Estimated': est_d
                })

                dp_folder = os.path.join(fold_log_dir, dp_id)
                os.makedirs(dp_folder, exist_ok=True)
                save_comparison_plot(
                    im_d, gt_d, ps_d, recon,
                    os.path.join(dp_folder, "comparison.png"),
                    dp_id, fold_name
                )
                print(f"   Done: {dp_id} | Actual: {act_d:.3f} | Est: {est_d:.3f}")
            except Exception as e:
                print(f"\nFail {dp_id}: {e}")
    ...
    if not df.empty:
        global_report = [
            "=" * 100,
            "SPQA SCIENTIFIC VALIDATION SUMMARY",
            "=" * 100,
            f"Checkpoint: {os.path.basename(model_path)}",
            "-" * 100,
        ]
        header = (f"{'Fold':<10} | {'N':<4} | {'Pearson r':<12} | {'MAE':<10} | "
                  f"{'Actual Dice':<20} | {'Estimated Dice':<20}")
        ...
        for fold in sorted(df['Fold'].unique()):
            ...
            if n > 1:
                ...
                global_report.append(
                    f"{fold:<10} | {n:<4} | {r_val:0.4f}       | {mae_val:0.4f}     | "
                    f"{act_mu:0.3f} +/- {act_std:0.3f}       | {est_mu:0.3f} +/- {est_std:0.3f}"
                )

                fold_stats_path = os.path.join(log_base_dir, fold, f"{fold}_stats.txt")
                with open(fold_stats_path, 'w') as ff:
                    ff.write(f"Pearson r: {r_val:0.4f}\n"
                             f"MAE: {mae_val:0.4f}\n"
                             f"Actual: {act_mu:0.4f} +/- {act_std:0.4f}\n"
                             f"Est: {est_mu:0.4f} +/- {est_std:0.4f}")
        ...
        global_report.append("-" * 100)
        global_report.append(
            f"{'OVERALL':<10} | {len(df):<4} | "
            f"{g_r:0.3f} +/- {np.std(fold_r_vals):0.3f} | "
    ...
    print(f"\nAudit complete. Results stored in: {log_base_dir}")
